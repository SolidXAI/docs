...
|---------------------|-------------|----------------------|
| **Short Text** | Names, IDs, status values, email addresses | `required`, `unique`, `min`, `max`, `isUserKey` |
| **Long Text** | Descriptions, notes, tokens | `required` |
| **Boolean** | Yes/No flags (part payment allowed, is overdue) | `required`, `defaultValue` |
| **Decimal** | Money amounts, precise calculations | `required`, `defaultValue`, `min`, `max` |
| **Integer** | Counters (overdue days) | `required`, `defaultValue` |
| **Datetime** | Dates and timestamps | `required` |
| **Media (Single)** | File uploads (Excel files) | `mediaTypes`, `mediaMaxSizeKb`, `mediaStorageProviderUserKey` |
| **Relation** | Links between models | `relationType`, `relationCoModelSingularName`, `relationCoModelFieldName`, `relationCascade` |

#### Important Configuration Notes

**Relation Types:**
- **One-to-Many**: Parent has multiple children (Payment Collection → Payment Collection Items)
- **Many-to-One**: Child belongs to parent (Payment Collection Item → Student)
- Always set both sides when `relationCreateInverse` is true

**Cascade Options:**
- **cascade**: Delete children when parent is deleted
- Use for tightly coupled data (Payment Collection and its Payment Collection Items)

**Audit Tracking:**
- Enable on fields that need change history
- Typically enabled for business-critical fields (amounts, status, due dates)
- Not needed for computed or temporary fields (OTP, tokens)
...
- Typically applied to status fields, foreign keys, and unique identifiers

**Media Storage:**
- `default-filesystem`: Stored on local server disk
- Set appropriate size limits (5MB for Excel files)
...
2. Unique constraints are set where needed (Student Login ID, Payment Collection ID)
...
7. Index fields are set for frequently queried fields
...
- The generated APIs will allow you to perform CRUD operations on the Student, Payment Collection, Payment Collection Item, and Payment Collection Item Detail models. You can view the API documentation in the Swagger UI provided by SolidX at `/docs`.
- The generated UI components will provide a basic interface for managing payment collections and viewing student records. You can access these components through the model menu items which will be automatically added to the menu within the Fees Portal module.
...
9. You can access the generated UI components through the model menu items within the Fees Portal module.
