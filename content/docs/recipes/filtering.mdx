...
In SolidX, filtering is a shared query contract used by both REST endpoints and backend services.

- Fields define what part of the record is being filtered.
- Operators define how values are compared.
- Logical operators such as `$and`, `$or`, and `$not` define how conditions are combined.

Learning the filter shape once lets you use the same query pattern across API calls, service methods, and higher-level data access flows.

</Callout>

SolidX provides a powerful filtering syntax that works consistently across two layers:

- **REST API** - pass filters as query-string parameters when calling the [Retrieve endpoints](/docs/developer-docs/rest-apis/retrieve).
- **CRUD Service** - pass a `filters` object inside the DTO when calling [`find()`](/docs/developer-docs/extending/backend-customization/crud-service#4-findfilterdto-context) or [`findOne()`](/docs/developer-docs/extending/backend-customization/crud-service#5-findoneid-query-context) from backend code.
...
<Callout type="info" title="Default Combination">
...
`$and`, `$or`, and `$not` can be nested inside one another for complex logic.
