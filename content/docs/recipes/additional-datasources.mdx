...
In SolidX, each datasource is a named database boundary that the application can discover and use at runtime.

- The default datasource uses the implicit `default` TypeORM connection.
- Additional datasources use unique connection names such as `secondary`.
- Environment variables, entity registration, repositories, and migrations all need to point to the same datasource name.

Adding another datasource creates a separate database context, not only another set of connection settings.
...
<Callout type="info" title="Additional Datasource Shape">
...
<Callout type="note" title="Key Points">
...
<Callout type="tip" title="Scoping Tip">
...
<Callout type="warning" title="Entity Registration">
