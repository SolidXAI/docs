...
sequenceDiagram
    ...
    Frontend->>Backend: POST /api/payment-collection-item/cancel-payment-collection  with item IDs or filters
      ...
      <Dialog header="Confirm Cancellation" visible={visible} style={{ width: "30vw" }} modal onHide={handleClose}>
        ...
        <div>
          <div>
            ...
            <label htmlFor="optionSelected">Cancel {selectedRecords?.length || 0} selected record(s)</label>
          </div>
          <div>
            <RadioButton inputId="optionFiltered" value="filtered" onChange={(e) => setOption(e.value)} checked={option === "filtered"} />
            <label htmlFor="optionFiltered">Cancel all records matching the current filter</label>
          </div>
        </div>
        <div>
