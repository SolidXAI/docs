...
<Callout type="info" title="Already have PostgreSQL?">
...
`solidctl` is installed globally (e.g. `npm i -g @solidxai/solidctl`) so the command is available directly from any directory.
...
4. **Other Control Plane CLI Commands**
   - **solidctl seed** — Seed the database with canonical models, settings, and system users from configuration files.
   - **solidctl build** — Builds both the frontend and backend, and updates the CLI to point to the latest codebase.
   - **solidctl upgrade** — Upgrade SolidX dependencies to their latest stable versions.
...
Run `solidctl create-app` as described [above](#solidx-scaffolding-script) and follow the prompts below to configure your application:
...
The `solidctl seed` command automates this process by populating the database with all these essentials, making the application ready for immediate use.
