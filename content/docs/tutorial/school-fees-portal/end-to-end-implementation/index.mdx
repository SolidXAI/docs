...
import { Building2, Globe, FileSpreadsheet, CreditCard } from 'lucide-react';
...
<Cards>
  <Card icon={<Building2 />} title="Institute Onboarding" href="./institute_onboarding.md">
    Register an institute, configure fee types, invite admin users, and set up branding for the student portal.
  </Card>
  <Card icon={<Globe />} title="Activate Institute Portal" href="./activate_institute.md">
    Provision DNS and web server configuration to make an institute's student portal live and accessible.
  </Card>
  <Card icon={<FileSpreadsheet />} title="Initiate Payment Collection" href="./initiate_payment.md">
    Upload a bulk Excel file to generate payment requests for multiple students across multiple fee types in one go.
  </Card>
  <Card icon={<CreditCard />} title="Student Payment Portal" href="./making_payment.md">
    Allow students and parents to view dues, make online payments via Stripe, and track payment history through a dedicated portal.
  </Card>
</Cards>
