---
title: How Testing Hub Fits Your Product
icon: "book-open"
description: A simple mental model for using Testing Hub to protect the customer journeys that matter.
---


Testing Hub follows the same simple cycle your product team already uses when preparing a release: decide what matters, test it, understand the result, and act when needed.

## The mental model

| In Testing Hub | What it means for your product |
|---|---|
| **Product** | The website, service, or area of your business that you want to protect. |
| **Customer journey** | An important task someone must be able to complete, such as signing in or placing an order. |
| **Test scenario** | A saved test for one customer journey, created with the MCP AI assistant. For example: confirm that a customer can sign in. |
| **Test run** | The occasion when Testing Hub runs one or more selected test scenarios. |
| **Result** | A clear record of which customer journeys passed and which need attention. |

## A typical release flow

1. Choose the customer journeys that must work for the release.
2. Describe those journeys to the MCP AI assistant so it can create the test scenarios.
3. Run those tests before approving the release.
4. Review the result: all clear, or identify the journey that needs attention.
5. Share the result with the people who need to act.
6. Keep the same tests running regularly so problems are found sooner next time.

<Callout type="tip" title="Start small">
  Begin with the three to five journeys that would most affect customers or the business if they failed. You can add more as the product grows.
</Callout>

## Example

For an online school-fee portal, the first tests might be:

- A parent can sign in.
- A parent can see the amount due.
- A parent can complete a payment.
- The school team can see that payment in the portal.

These tests give the product owner a fast, repeatable answer to whether the most important experience is ready for customers.
