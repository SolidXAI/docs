...
<Callout type="info" title="Mental Model">
  ...
  SolidX generates the runtime plumbing for background jobs, but the queue contract and the broker-specific handlers still need to be modeled explicitly.

  - Publishers enqueue jobs without blocking the request path.
  - Subscribers consume jobs asynchronously using the selected broker.
  - Queue definitions, payload contracts, and provider wiring decide how the work is routed and processed.

  This page is the bridge between business logic that should happen later and the runtime infrastructure that processes it reliably.
