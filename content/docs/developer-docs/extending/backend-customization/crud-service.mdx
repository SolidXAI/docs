...
title: CRUD Service
icon: "database"
description: Reference for creating and customizing CRUD services in SolidX.
...
Draft/publish filtering (when enabled on the model) goes through `filters`, not a top-level shortcut - see [Draft & Publish Workflow](/docs/recipes/draft-publish#querying).
...
<Tabs defaultValue="single-write">
  <TabItem value="single-write" label="Single-Record Writes">
...
For the full list of filter operators and examples, see the [Filtering Data](/docs/recipes/filtering) recipe.
...
The `query` parameter accepts the same filter syntax as `find()`. See the [Filtering Data](/docs/recipes/filtering) recipe for all available operators.
...
```
  ...
  <TabItem value="recovery" label="Recovery">
