...
<details open>
 <summary> Example: Country Seeder </summary>
...
<Steps>
  <Step>
    **Discovery** - On application bootstrap, SolidX scans all providers for the `@SolidSeeder` decorator and registers them in the `SolidRegistry`.
  </Step>
  <Step>
    **Lookup** - When you run the `seed` CLI command, it looks up the seeder by class name from the registry.
  </Step>
  <Step>
    **Execution** - The seeder's `seed()` method is called.
  </Step>
</Steps>
