...
summary: Covers the built-in SMS providers shipped with SolidX and the implementation model for custom providers using the `ISMS` interface.
...
## Steps to Create a Custom SMS Provider
...
export class CustomSMSService implements ISMS {
  ...
  async sendSMSSynchronously(message: QueueMessage<any>): Promise<any> {
    // Add your custom SMS provider integration here.
...
| Step | What you do |
|---|---|
| 1 | Implement `CustomSMSService` (implements `ISMS`, decorated with `@SmsProvider()`) |
| 2 | Register it in your module |
| 3 | Use `SmsServiceFactory` to send template-based SMS |
