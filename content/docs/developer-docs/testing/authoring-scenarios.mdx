...
- `timeoutMs`: A limit on how long the whole step may take.

`timeoutMs` here is not the same as `timeoutMs` inside a UI step's `with` block. Use this one to bound a step that could hang, such as `api.request`. Use the one inside `with` to control how long a UI step waits for an element, because it reports which selector it was waiting for. See [Timeouts and Waiting](/docs/developer-docs/testing/timeouts).
...
- `${secret:key}` for credentials held in the secret store.
...
Use `${secret:key}` when a scenario needs a credential — an API key, a password, a bearer token — instead of putting the value in metadata or passing it through an environment variable.

The key is the secret's key exactly as stored. Dots are part of the key, not a path:
...
```json
{
  "when": {
    ...
    "with": {
      "method": "POST",
      "url": "${env:API_BASE_URL}/api/orders",
      "headers": {
        "Authorization": "Bearer ${secret:payments.apiKey}"
      }
    }
  }
}
...
How secrets differ from `${env:...}`:

- **They are resolved on the server, from the store.** A run request cannot supply or override a secret value, so a scenario always uses the credential the store holds.
- **They keep their declared type.** A secret stored as `json` resolves to an object and one stored as `number` to a number, when the token is the entire value. Inside a longer string it is converted to text, as any token is.
- **Only what you reference is resolved.** The engine scans the scenarios selected for the run and fetches just those keys — a scenario never receives secrets belonging to another scenario in the same run.
- **A missing secret fails the run before any scenario starts**, rather than failing mid-suite. The same applies to a secret whose status is not `active`, which makes deactivating a secret an effective way to revoke it.

<Callout type="warning" title="Do not reference production credentials yet">

Resolved secret values are not currently redacted from test output. They can appear in console output, API logs, captured response bodies, error details, and the run events sent to Test Hub — which stores them.

Reference secrets belonging to test environments until output redaction ships.
