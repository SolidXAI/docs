...
- `${secret:key}` reads a credential from the secret store.
...
A credential held in the SolidX secret store and referenced from a scenario as `${secret:key}`.

Unlike `${env:...}` values, secrets are never supplied by the caller of a test run. They are resolved on the server from the store, decrypted only for the duration of the run, and returned with the type declared on the secret — a `json` secret resolves to an object, a `number` to a number.

Only the secrets a scenario actually references are resolved, and a scenario never receives secrets referenced by other scenarios in the same run.
