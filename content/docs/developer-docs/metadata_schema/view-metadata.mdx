...
In list and tree views, `many-to-one` renders as a single related value.

By default, the list column uses `DefaultRelationManyToOneListWidget`, which displays the related record's user key and opens the related form in a new tab when the cell is interactive.
...
| Attr | Purpose |
| --- | --- |
| No extra attrs | This widget derives the avatar label from the related model's user key field and renders an avatar badge instead of the default text link. |
...
By default, the list column uses `DefaultRelationOneToManyListWidget`, which shows the first related label and a `+N` indicator when additional related records are present.
...
<TabItem value="list-onetomany-alternative" label="Alternative widgets">
...
By default, the list column uses `DefaultRelationManyToManyListWidget`, which shows the first related label and a `+N` indicator when additional related records are present.
...
| Attr | Purpose |
| --- | --- |
| No extra attrs | This widget derives avatar labels from the related model's user key field and renders the first related record as an avatar with a `+N` count for the rest. |
...
By default, the list column uses `DefaultMediaSingleListWidget`, which shows an image/video/audio preview when possible and falls back to a file icon or download behavior for document-style assets.
...
<TabItem value="list-mediasingle-alternative" label="Alternative widgets">
...
By default, the list column uses `DefaultMediaMultipleListWidget`, which can open media in the lightbox, download document-style files, and show the full collection on demand.
...
<TabItem value="list-mediamultiple-alternative" label="Alternative widgets">
...
In list and tree views, `computed` does not use its own dedicated list widget family.

Instead, the list column delegates to the existing scalar renderers based on `computedFieldValueType`, such as text, numeric, boolean, date, or datetime columns.
...
| Concern | How it works |
| --- | --- |
| Default list widget | Delegated from `SolidComputedColumn` based on `computedFieldValueType` |
| Widget selection | Text-like values use the short-text column path; numeric values use the numeric column path; date-like and boolean values use their matching scalar columns |
| Typical field-node attrs | `name`, `label`, `sortable`, `filterable`, `isSearchable` |
...
<TabItem value="list-computed-alternative" label="Alternative widgets">
...
By default, `many-to-one` renders as an autocomplete-based relation picker in edit mode and as a plain related label in view mode.
...
| Attr | Purpose |
| --- | --- |
| No extra attrs | This widget derives avatar labels from the related model's user key field and renders the relation in a badge-style view mode. |
...
By default, `many-to-many` renders as a relation picker in edit mode and as an embedded list-style relation browser in view mode.
...
By default, `mediaSingle` renders as a single-file upload and preview experience in edit mode and as a preview/download surface in view mode.
...
<TabItem value="form-mediasingle-alternative" label="Alternative widgets">
...
By default, `mediaMultiple` renders as a multi-file upload and preview experience in edit mode and as a gallery-style preview/download surface in view mode.
...
<TabItem value="form-mediamultiple-alternative" label="Alternative widgets">
...
By default, `computed` renders as a read-only field and is hidden entirely while a new record is still being created.

The default edit and view path both use `DefaultComputedFormWidget`.
...
| Concern | How it works |
| --- | --- |
| Default edit widget | `DefaultComputedFormWidget` |
| Default view widget | `DefaultComputedFormWidget` |
| Widget selection | Applied automatically when `editWidget` or `viewWidget` is not provided |
| Typical field-node attrs | `name`, `label`, `description`, `showLabel` |
...
<TabItem value="form-computed-alternative" label="Alternative widgets">
