...
<Callout type="info" title="Mental Model">
  ...
  - <strong>Database:</strong> stores both application data and SolidX metadata.
    ...
    - <strong>Python 3.13+:</strong> required for the SolidX MCP server and agent tooling.
...
Python is required if you plan to install or run the **SolidX MCP server** or use the **SolidX agent**. These components are Python-based and depend on features available only in Python 3.13 and above.
...
# Reload shell, then install Python 3.13
pyenv install 3.13
pyenv global 3.13
...
On macOS you can also use Homebrew: `brew install python@3.13`. On Ubuntu, use the [deadsnakes PPA](https://launchpad.net/~deadsnakes/+archive/ubuntu/ppa) if 3.13 is not available in the default apt repos.
