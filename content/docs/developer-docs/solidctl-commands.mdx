...
| [`setup-app`](#setup-app) | Set up an already-bootstrapped SolidX project you've just cloned. |
...
- **Bootstrap:** `create-app`, `setup-app`, `build`
...
```bash
# Install, configure, build, seed, and start in one command
solidctl setup-app
```
...
Sets up an **already-bootstrapped** SolidX project you've just cloned — the counterpart to `create-app` for a project that already exists rather than one being scaffolded from scratch. Infers the project name and UI port from `solid-api`/`solid-ui`'s `package.json`, prompts only for the database connection, then installs dependencies, writes `.env`, verifies or creates the database, builds, seeds, and starts the dev servers.
...
| `--db-client <client>` | `PostgreSQL` | Database type: `PostgreSQL`, `MySQL`, or `MSSQL` |
| `--db-host <host>` | `localhost` | Database host |
| `--db-port <port>` | `5432` / `3306` / `1433` | Database port |
| `--db-name <name>` | `solidx_app_db` | Database name |
| `--db-username <username>` | `solidx_app_user` | Database username |
| `--db-password <password>` | `strongpassword` | Database password |
...
1. Reads project name and UI port from `package.json` — no prompt needed for either.
2. Writes `solid-api/.env` and `solid-ui/.env`:
   - If `solid-api/.env` already exists, it's left untouched (pass `--force-env` to regenerate).
   - If `solid-api/.env.example` exists, its non-secret values are carried over and only the gaps (and the database password) are prompted for.
   - Otherwise, only the database connection is prompted for. Project-specific settings the template doesn't know about (RabbitMQ, OAuth, SMTP, Redis, etc.) aren't inferred and need to be added manually afterward.
3. Verifies the database exists, or creates it.
4. Installs dependencies for `solid-api` and `solid-ui`.
5. Builds the project and sets up the Solid CLI, same as `solidctl build`.
6. Seeds metadata, settings, and the system user, same as `solidctl seed`.
7. Starts the dev servers, same as `solidctl start:dev`, unless `--no-start` was passed.

### Notes

- Requires both `solid-api/package.json` and `solid-ui/package.json` to define a `solidx:dev` script. If either is missing — a project that predates that script — `setup-app` fails fast and names the exact line to add.
- The API template currently ships only a PostgreSQL driver. Selecting `MySQL` or `MSSQL` still runs setup end to end, but `solid-api` is likely to fail to boot until that's addressed upstream; `setup-app` warns when this happens.
...
| `--ui-timeout-ms [number]` | Default wait for UI steps in ms; defaults to `process.env.UI_TIMEOUT_MS`, then the `uiTestDefaultTimeoutMs` setting, then `30000`. See [Timeouts and Waiting](/docs/developer-docs/testing/timeouts) |
| `--ui-navigation-timeout-ms [number]` | Default wait for `ui.goto` in ms; defaults to `process.env.UI_NAVIGATION_TIMEOUT_MS`, then the `uiTestNavigationTimeoutMs` setting, then `--ui-timeout-ms` |
