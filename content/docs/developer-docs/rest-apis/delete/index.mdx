...
Delete endpoints remove records from active results. When soft delete is enabled, the record is marked as deleted instead of being physically removed from the database.

</Callout>

## Deleting a Single Record

When soft delete is enabled on a model such as `fee-type`, the record is not permanently removed. Instead, the `deletedAt` and `deletedTracker` fields are populated.
...
The response confirms soft deletion and shows the tracking metadata returned by the API.
