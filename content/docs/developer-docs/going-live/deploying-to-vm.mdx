...
VM deployment is the most explicit hosting model in this section.

- You manage the machine, the processes, the reverse proxy, and the operating-system-level concerns directly.
- Choose this path when your team wants maximum control over the host.
- Expect to handle process lifecycle, networking, SSL, logs, and patching yourself.

A VM gives you the most control, but it also gives you the most operational responsibility.
...
Before you begin, complete the [Prerequisites](/docs/developer-docs/prerequisites) guide on the VM. That includes installing Node.js, Git, PostgreSQL, and the required base tooling.
...
Keep `.env` files secure and out of version control. Add `.env` to your `.gitignore`.
...
Before automating the deployment, verify that both applications build correctly on the server.

<Tabs defaultValue="api">
  <TabItem value="api" label="Verify Backend">
  ...
  `solid-api` will eventually run behind PM2 and Nginx. This step confirms the backend builds cleanly on the server.

  </TabItem>
  <TabItem value="ui" label="Verify Frontend">
  ...
  Verify that the `dist` directory is created successfully. The frontend will be served directly by Nginx as static files.
...
<Tabs defaultValue="api">
  ...
  **Backend (`/etc/nginx/sites-available/api.your_domain.com`):**
  ...
  <TabItem value="ui" label="Frontend UI">
...
Before calling the VM deployment production-ready, confirm:

- PM2 restarts correctly after reboot.
- Nginx configuration validates cleanly.
- SSL certificates are active and renew successfully.
- Logs are rotated and monitored.
- The database, API, and frontend all use the correct production environment values.

</Callout>

You have successfully deployed and secured your SolidX application on a VM.
